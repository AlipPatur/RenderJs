...
async function renderWorld(ctx, textures, world) {
    console.log(`Rendering world ${world.name}, with ${world.blocks.length} tiles`);
                ...
                { //Object
                    if (world.objects.length > index) {
                        ...
                        if (item.m_id % 2 == 0) {
                            ...
                            // Shadow
                            tempCtx.drawImage(sprite, (m_default_texture_x + offset_x) * 32, (m_default_texture_y + offset_y) * 32, 32, 32, 0, 0, 19, 19);
                            tempCtx.globalCompositeOperation = 'source-in';
                            tempCtx.fillStyle = 'rgba(0, 0, 0, 0.56)';
                            ...
                            let offset_x = item.m_seed_base;
                            let offset_y = 0;

                            const tempCanvas = createCanvas(16, 16);
                            const tempCtx = tempCanvas.getContext('2d');
                            tempCtx.drawImage(textures["seed"], (offset_x) * 16, (offset_y) * 16, 16, 16, 0, 0, 16, 16);
                            tempCtx.globalCompositeOperation = 'source-in';
                            tempCtx.fillStyle = 'rgba(0, 0, 0, 0.56)';
                ...
                { //Object
                    if (world.objects.length > index) {
                        ...
                        if (item.m_id % 2 == 0) {
                            ...
                            ctx.drawImage(sprite, (m_default_texture_x + offset_x) * 32, (m_default_texture_y + offset_y) * 32, 32, 32, x, y, 19, 19);
                            ...
                            if (item.m_id % 2 == 1) {
                                ...
                                const blendedCanvas = blendColor(textures["seed"], (offset_x) * 16, (offset_y) * 16, 16, 16, 16, 16, seed_base_color);
                                ctx.drawImage(blendedCanvas, x, y);
                            }

                            if (item.m_id % 2 == 1) {
                                offset_x = item.m_seed_overlay;
                                offset_y = 1;
                                ...
                                const blendedCanvas = blendColor(textures["seed"], (offset_x) * 16, (offset_y) * 16, 16, 16, 16, 16, seed_overlay_color);
                                ctx.drawImage(blendedCanvas, x, y);
                            }
    ...
    function setTransparent(sprite, sx, sy, sw, sh, dw, dh, value) {
        const tempCanvas = createCanvas(dw, dh);
        const tempCtx = tempCanvas.getContext('2d');
